...
TEST_F(MemkindMemtierDynamicTest, test_tier_policy_dynamic_threshold_ctl_set)
{
    int res = memtier_builder_add_tier(m_builder, MEMKIND_DEFAULT, 1);
    ASSERT_EQ(0, res);
    res = memtier_builder_add_tier(m_builder, MEMKIND_REGULAR, 2);
...
TEST_F(MemkindMemtierDynamicTest,
       test_tier_policy_dynamic_threshold_ctl_set_only_val)
{
    int res = memtier_builder_add_tier(m_builder, MEMKIND_DEFAULT, 1);
    ASSERT_EQ(0, res);
    res = memtier_builder_add_tier(m_builder, MEMKIND_REGULAR, 2);
...
TEST_F(MemkindMemtierDynamicTest,
       test_tier_policy_dynamic_threshold_ctl_set_internals)
{
    int res = memtier_builder_add_tier(m_builder, MEMKIND_DEFAULT, 1);
    ASSERT_EQ(0, res);
    res = memtier_builder_add_tier(m_builder, MEMKIND_REGULAR, 2);
    ...
    float degree = 0.25;
    res =
        memtier_ctl_set(m_builder, "policy.dynamic_threshold.degree", &degree);
    ASSERT_EQ(0, res);
    float trigger = 0.1;
...
TEST_F(MemkindMemtierDynamicTest,
       test_tier_policy_dynamic_threshold_ctl_failure_invalid_name)
{
    int res = memtier_builder_add_tier(m_builder, MEMKIND_DEFAULT, 1);
    ASSERT_EQ(0, res);
    res = memtier_builder_add_tier(m_builder, MEMKIND_REGULAR, 2);
    ASSERT_EQ(0, res);
    size_t val = 1000;
    res = memtier_ctl_set(m_builder, "aa", &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder, "policy.ddd.d41", &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder, "policy.dynamic_threshold..ff", &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder, "policy.dynamic_threshold.[h8]]", &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder, "policy.dynamic_threshold.thresholds[[]",
                          &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder, "policy.dynamic_threshold.thresholds[0a].",
                          &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder,
                          "policy.dynamic_threshold.thresholds[0]..da12", &val);
    ASSERT_NE(0, res);
    res = memtier_ctl_set(m_builder,
                          "policy.dynamic_threshold.thresholds[0].fafa", &val);
    ASSERT_NE(0, res);
}
...
TEST_F(MemkindMemtierDynamicTest,
       test_tier_policy_dynamic_threshold_ctl_failure_negative_thres_degree)
{
    int res = memtier_builder_add_tier(m_builder, MEMKIND_DEFAULT, 1);
    ASSERT_EQ(0, res);
    res = memtier_builder_add_tier(m_builder, MEMKIND_REGULAR, 2);
...
TEST_F(MemkindMemtierThresholdSizesTest, test_final_alloc_sizes)
{
    ...
    const size_t alloc_sizes[alloc_sizes_num] = {1024, 1024, 1024, 1024, 1024,
                                                 1024, 1024, 1024, 8192};
    ...
    for (i = 0; i < alloc_num; ++i) {
        ...
        if (memkind_detect_kind(ptr) == MEMKIND_DEFAULT) {
    ...
    // check if most of small allocations goes to first tier
    const float min_small_allocs = 0.85; // 85%

    float small_allocs_in_default =
        (float)std::count_if(default_sizes.begin(), default_sizes.end(),
                             [](int i) { return i == 1024; });
    ASSERT_GE(small_allocs_in_default / default_sizes.size(), min_small_allocs);

    float small_allocs_in_regular =
        (float)std::count_if(regular_sizes.begin(), regular_sizes.end(),
                             [](int i) { return i == 1024; });
    ASSERT_LE(small_allocs_in_regular / regular_sizes.size(), 1 - min_small_allocs);
