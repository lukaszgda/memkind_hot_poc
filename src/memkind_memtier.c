                                          ...
                                          size_t size)
{
    ...
    char str[256] = {0};
    char *p = str;
    // p += sprintf(p, "get_kind, size: %ld, ", size);
#endif
...
#if RRR
        // size_t s = memtier_kind_allocated_size(cfg[i].kind);
        // p += sprintf(p, "tier: %d size: %ld, val: %ld, ", i, s,
        // thres[i].val);
#endif
...
#if RRR
    // p += sprintf(p, "-> %d", dest_tier);
    // log_err("%s", (char *)str);
#endif

    // for every trigger, check if distance between actual vs desired ratio
    // between threshold's tiers is above TRIGGER level and if so, change
    // threshold by CHANGE_VAL

#if RRR
    p = str;
    p += sprintf(p, "size: %ld, ", size);
#endif
    ...
    for (i = 0; i < memory->size - 1; ++i) {
        ...
        if (prev_alloc_size > 0) {
            ...
            float diff =
                1.0f - actual_ratio / cfg[i].kind_ratio * cfg[i + 1].kind_ratio;
        ...
        if (change != 0) {
            ...
            thres[i].val = (new_val > thres[i].max) ? thres[i].max
                : (new_val < thres[i].min)          ? thres[i].min
                                                    : new_val;
        }

#if RRR
        size_t s = memtier_kind_allocated_size(cfg[i].kind);
        p += sprintf(p, "tier %d: size: %ld, change: %d, th: %ld, ", i, s,
                     change, thres[i].val);
#endif
    }

#if RRR
    size_t s = memtier_kind_allocated_size(cfg[memory->size - 1].kind);
    p += sprintf(p, "tier %d: size: %ld", memory->size - 1, s);
    log_err("%s", (char *)str);
#endif
...
MEMKIND_EXPORT int
memtier_builder_add_threshold(struct memtier_builder *builder, size_t init)
{
    return memtier_builder_add_threshold_min_max(builder, init, 0, SIZE_MAX);
}
